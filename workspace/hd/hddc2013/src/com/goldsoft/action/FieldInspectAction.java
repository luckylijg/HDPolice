...
public class FieldInspectAction extends BaseAction{
	...
	public String list()
	{
		...
		this.pager = this.complaintService.getListByPage(1, userid);
		return "list";
	}
	...
	{
		int userid = this.getLoginUser().getUserid();
		list = this.complaintService.getListPrint(1, userid);
		return "listlistprint";
	}
	
	public String inAdd()
	{
		//nextcode = this.codeService.getNextCode(2);
	...
	public String add()
		{
		//System.out.println("leaderapproved :"+leaderapproved+"           leader"+leader);
		Complaint com = new Complaint();
		com.setType(1);
		sno = this.codeService.getNextCode(2);
		com.setSno(sno);
		com.setTime(time);
	...
	public String getUsers() {
				...
				String userString = " ";
				for(User u:users){
					userString = userString + u.getRealname() + " ";
				}
				return userString;
	}
	...
	public void setUsers(String users) {
		...
		Set<User> users1 = new HashSet<User>(0);
		String userString[] = users.split(" ");
		List<User> list = this.userService.getlist();
		for(String u:userString)
		{
			for(User user: list)
			{
				if (u.equals(user.getRealname())) {
					users1.add(user);
				}
			}
		}
