...
<%--     <div style="{position:absolute;top:4%;right:7%;}">
  ...
  <input type="image" onclick="javascript:docucument.getElementById('fromId').submit();" src="${ctx }/images/print.png"/>  
     ...
     <div class="R-CON">
     <form  id="from119" action="manage/department/department!qianfen.action" method="post" >
<table  style="width:89%;" align="center">
  		...
  		被考核单位<select name="departmentName">
  					    <c:forEach items="${list }" var="depart">
  					    <option value="${depart.departmentid }" <c:if test="${depart.departmentid==param.departmentName}">selected</c:if>>${depart.name }</option>		
  					    </c:forEach>
 ...
 <form  id="fom119" action="manage/department/department!qianfen.action" method="post" >
  <input type="hidden" name="year" value=${param.year }>
  <input type="hidden" name="month" value=${param.month }>
  <input type="hidden" name="departmentName" value=${departmentName }>
